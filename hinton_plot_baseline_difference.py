...
"""
Generates a Hinton plot of the difference of the baseline R2 and R2 of two feature and one feature loss of the respective timepoint.
It takes two csv files, one features and two feature loss.

Usage:
    python3 heatmap.py  --two_feature /path/to/twofeature.csv  --one_feature /path/to/onefeature.csv --outdir /path/to/output_dir
"""
...
def parse_args():
    ...
    parser.add_argument("--one_feature", required=True, help="Path to the single-feature removal CSV.")
    parser.add_argument("--outdir",  help="Directory to save Hinton plots.")
...
# two feature loss values
def prepare_two_feature(df_two):
    baseline = (df_two[df_two["removed_feature"].isna()].set_index("timepoint")["CV_mean_R2"])
...
# single feature loss values
def prepare_single_feature(df_single):
    baseline_single = (df_single[df_single["removed_feature"].isna()].set_index("timepoint")["CV_mean_R2"])
...
def main():
    ...
    df_two = pd.read_csv(args.two_feature)
    df_single = pd.read_csv(args.one_feature)
    ...
    for tp in timepoints:
        ...
        outpath = os.path.join(args.outdir, f"hinton_{tp}.png")
